...
void agregar() {
	...
	for (i; i <= 2; i++) {
		if (articulo[i].anioarticulo == NULL && i <= 2) {
			...
			do {
				cout << "\n\tMANUAL: ingresa los datos necesarios para poder completa el código utilizando los dígitos y información correspondiente\n";
...
void borrar() {
	int borrarcdg;
	int i = 0;
	cout << "MANUAL: el borrador de juegos es configurado por los dígitos que utilizaste en la lista de artículos\n";
...
void mostrar() {
	...
	if (articulo[0].anioarticulo != NULL) {
		cout << "----MANUAL: aquí esta los datos que ingresaste----\n";
...
void comprar() {
	...
	if (articulo[i].anioarticulo == NULL) {
		...
		cout << "MANUAL: para poder comprar los juegos tendrás que ingresar los dígitos correspondientes\n";
